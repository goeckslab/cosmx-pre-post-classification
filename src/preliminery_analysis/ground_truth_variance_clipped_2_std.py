...
plot_save_path = Path("plots", "variance", "clipped", "2_std")
data_save_path = Path("data", "outlier",  "2_std")
...
for file in Path("data/csv").iterdir():
    try:
        ...
        clipped = np.sum(np.abs(df - df.mean()) > (2 * df.std()))
        ...
        try:
            ...
            df = df.clip(lower=df.mean() - (2 * df.std()), upper=df.mean() + (2 * df.std()), axis=1)
